...
import com.flash.worker.module.business.view.adapter.ComplaintModifyHistoryAdapter
import kotlinx.android.synthetic.main.activity_complaint_modify_history.*
import kotlinx.android.synthetic.main.rv_complaint_modify_his_cell.*

/*
 * -----------------------------------------------------------------
 * Copyright (C) 2020-2080, by Victor, All rights reserved.
 * -----------------------------------------------------------------
 * File: ComplaintModifyHistoryActivity
 * Author: Victor
 * Date: 2020/12/17 20:41
 * Description: 
 * -----------------------------------------------------------------
 */
class ComplaintModifyHistoryActivity: BaseActivity(),View.OnClickListener,
    SwipeRefreshLayout.OnRefreshListener,AdapterView.OnItemClickListener {

    companion object {
        fun  intentStart (activity: AppCompatActivity,complaintNo: String?) {
            var intent = Intent(activity, ComplaintModifyHistoryActivity::class.java)
    ...
    var mComplaintModifyHistoryAdapter: ComplaintModifyHistoryAdapter? = null
    var currentWorkImagePositon = 0//当前工作情景图片浏览位置


    override fun getLayoutResource() = R.layout.activity_complaint_modify_history
    ...
    fun initialize () {
        ...
        mComplaintModifyHistoryAdapter = ComplaintModifyHistoryAdapter(this,this)

        val animatorAdapter
                = AlphaAnimatorAdapter(mComplaintModifyHistoryAdapter,mRvHistory)
    ...
    fun showDisputeHistoryData (datas: DisputeHistoryReq) {
        mComplaintModifyHistoryAdapter?.showData(datas.data,mTvNoData,mRvHistory)
    }
    ...
    override fun onRefresh() {
        mComplaintModifyHistoryAdapter?.clear()
        mComplaintModifyHistoryAdapter?.setFooterVisible(false)
        mComplaintModifyHistoryAdapter?.notifyDataSetChanged()
    ...
    override fun onItemClick(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
        ...
        when (view?.id) {
            R.id.mClCertificateRoot -> {
                ...
                var count = mComplaintModifyHistoryAdapter?.getDatas()?.size ?: 0
                if (categoryPosition < count) {
                    ViewImageActivity.intentStart(this,
                        getViewImageUrls(mComplaintModifyHistoryAdapter?.getDatas()?.get(categoryPosition)?.complaintPics),
                        position,
