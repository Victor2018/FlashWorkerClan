...
import com.flash.worker.module.business.view.adapter.TalentEmployingAdapter
import kotlinx.android.synthetic.main.fragment_talent_employing.*
 ...
 * -----------------------------------------------------------------
 * File: TalentEmployingFragment
 * Author: Victor
 * Date: 2020/12/25 12:29
 * Description: 人才 已雇用
 * -----------------------------------------------------------------
 */
class TalentEmployingFragment: BaseFragment(), SwipeRefreshLayout.OnRefreshListener,
        LMRecyclerView.OnLoadMoreListener,AdapterView.OnItemClickListener, OnTradePasswordListener {

    companion object {
        fun newInstance(): TalentEmployingFragment {
            return TalentEmployingFragment()
        }
    }
    ...
    var mTalentEmployingAdapter: TalentEmployingAdapter? = null
    var currentPosition: Int = -1
    var passwordErrorCount: Int = 0//密码输入错误次数

    override fun getLayoutResource() = R.layout.fragment_talent_employing
    ...
    fun initialize () {
        ...
        mTalentEmployingAdapter = TalentEmployingAdapter(activity!!,this)
        val animatorAdapter
                = AlphaAnimatorAdapter(mTalentEmployingAdapter,mRvEmploying)
    ...
    fun initData () {
        currentPage = 1
        sendTalentEmployingRequest()
    }

    fun subscribeUi() {
        talentJobVM.talentEmployingData.observe(viewLifecycleOwner, Observer {
            mSrlRefresh.isRefreshing = false
            when(it) {
                is HttpResult.Success -> {
                    showTalentEmployingData(it.value)
                }
                is HttpResult.Error -> {
        ...
        employmentVM.cancelSignUpData.observe(viewLifecycleOwner, Observer {
            ...
            when(it) {
                is HttpResult.Success -> {
                    ...
                    mTalentEmployingAdapter?.removeItem(currentPosition)
                    mTalentEmployingAdapter?.notifyItemRemoved(currentPosition)
        ...
        talentJobVM.settlementDateData.observe(viewLifecycleOwner, Observer {
            ...
            when(it) {
                is HttpResult.Success -> {
                    var data = mTalentEmployingAdapter?.getItem(currentPosition)
                    TalentEmployingActivity.intentStart(activity as AppCompatActivity,it.value.data,data)
                }
                is HttpResult.Error -> {
                    ToastUtils.show(it.message.toString())
                }
    ...
    fun sendTalentEmployingRequest () {
        if (!App.get().hasLogin()) {
        ...
        var body = TalentEmployingParm()
        body.pageNum = currentPage

        talentJobVM.fetchTalentEmploying(token,body)
    }
    ...
    fun showTalentEmployingData (datas: TalentEmployingReq) {
        mTalentEmployingAdapter?.showData(datas.data,mTvNoData,mRvEmploying,currentPage)
    }
    ...
    override fun onRefresh() {
        currentPage = 1
        mTalentEmployingAdapter?.clear()
        mTalentEmployingAdapter?.setFooterVisible(false)
        mTalentEmployingAdapter?.notifyDataSetChanged()
    ...
    override fun onItemClick(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
        ...
        var data = mTalentEmployingAdapter?.getItem(position)
        ...
        when (view?.id) {
            ...
            R.id.mTvDetail -> {
                ...
                if (taskType == 1) {
                    ...
                    TalentTaskEmployingActivity.intentStart(activity as AppCompatActivity,data)
                }
            }
        }
    }
    ...
    override fun OnTradePassword(tradeAmount: Double, tradePassword: String?) {
        var id = mTalentEmployingAdapter?.getItem(currentPosition)?.id
