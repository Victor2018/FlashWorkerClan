...
import com.flash.worker.module.business.view.adapter.EmployerDisputeHandlingAdapter
 ...
 * -----------------------------------------------------------------
 * File: EmployerDisputeHandlingActivity
 * Author: Victor
 * Date: 2020/12/17 20:41
 * Description: 
 * -----------------------------------------------------------------
 */
class EmployerDisputeHandlingActivity: BaseActivity(),View.OnClickListener, SwipeRefreshLayout.OnRefreshListener,
    LMRecyclerView.OnLoadMoreListener,AdapterView.OnItemClickListener, OnTradePasswordListener {

    var mLoadingDialog: LoadingDialog? = null
    var mEmployerDisputeHandlingAdapter: EmployerDisputeHandlingAdapter? = null
    var currentPage = 1
    ...
    companion object {
        fun  intentStart (activity: AppCompatActivity) {
            var intent = Intent(activity, EmployerDisputeHandlingActivity::class.java)
            activity.startActivity(intent)
        }
    }

    override fun getLayoutResource() = R.layout.activity_employer_dispute_handling
    ...
    fun initialize () {
        ...
        mEmployerDisputeHandlingAdapter = EmployerDisputeHandlingAdapter(this,this)

        val animatorAdapter
                = AlphaAnimatorAdapter(mEmployerDisputeHandlingAdapter,mRvDisputeHandling)
    ...
    fun subscribeUi() {
        disputeVM.employerDisputeData.observe(this, Observer {
        ...
        disputeVM.cancelComplaintData.observe(this, Observer {
            ...
            when(it) {
                is HttpResult.Success -> {
                    ...
                    currentPage = 1
                    mEmployerDisputeHandlingAdapter?.clear()
                    mEmployerDisputeHandlingAdapter?.setFooterVisible(false)
                    mEmployerDisputeHandlingAdapter?.notifyDataSetChanged()
        ...
        disputeVM.employerDeleteDisputeData.observe(this, Observer {
            mLoadingDialog?.dismiss()
            when(it) {
                is HttpResult.Success -> {
                    ToastUtils.show("删除记录成功")

                    mEmployerDisputeHandlingAdapter?.removeItem(currentPosition)
                    mEmployerDisputeHandlingAdapter?.notifyItemRemoved(currentPosition)

                    if (mEmployerDisputeHandlingAdapter?.getContentItemCount() == 0) {
    ...
    fun sendDisputeRequest () {
        ...
        val body = EmployerDisputeParm()
        body.pageNum = currentPage

        disputeVM.fetchEmployerDispute(token,body)
    }
    ...
    fun sendCancelComplaintRequest (tradePassword: String?) {
        ...
        body.complaintNo = mEmployerDisputeHandlingAdapter?.getItem(currentPosition)?.complaintNo
    ...
    fun sendDeleteComplaintRequest () {
        ...
        val body = EmployerDeleteDisputeParm()
        body.complaintNo = mEmployerDisputeHandlingAdapter?.getItem(currentPosition)?.complaintNo

        disputeVM.employerDeleteDispute(token,body)
    }
    ...
    fun showDisputeData (datas: EmployerDisputeReq) {
        mEmployerDisputeHandlingAdapter?.showData(datas.data,mTvNoData,mRvDisputeHandling,currentPage)
    }
    ...
    fun showCancelComplaintTipDlg () {
        var data = mEmployerDisputeHandlingAdapter?.getItem(currentPosition)
        var employerCreditAmount = data?.employerCreditAmount
        ...
        commonTipDialog.mContent = "尊敬的雇主：\n" +
                ...
                "${employerCreditAmount}元；结算已预付报酬:${prepaidAmount}元。"
    ...
    fun showSetTransactionPwdDlg () {
        ...
        commonTipDialog.mOnDialogOkCancelClickListener = object : OnDialogOkCancelClickListener {
            override fun OnDialogOkClick() {
                NavigationUtils.goSetTransactionPasswordActivity(this@EmployerDisputeHandlingActivity)
            }
    ...
    fun showErrorPasswordTipDlg () {
        ...
        commonTipDialog.mOnDialogOkCancelClickListener = object : OnDialogOkCancelClickListener {
            ...
            override fun OnDialogCancelClick() {
                ...
                NavigationUtils.goVerifyIdentidyActivity(this@EmployerDisputeHandlingActivity)
            }
    ...
    fun showAuthTipDlg () {
        ...
        commonTipDialog.mOnDialogOkCancelClickListener = object : OnDialogOkCancelClickListener {
            override fun OnDialogOkClick() {
                NavigationUtils.goRealNameActivity(this@EmployerDisputeHandlingActivity)
            }
    ...
    override fun onItemClick(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
        ...
        when (view?.id) {
            ...
            R.id.mTvHandleDetail -> {
                var complaintNo = mEmployerDisputeHandlingAdapter?.getItem(position)?.complaintNo
                EmployerHandlingDetailActivity.intentStart(this,complaintNo)
            }
            else -> {
