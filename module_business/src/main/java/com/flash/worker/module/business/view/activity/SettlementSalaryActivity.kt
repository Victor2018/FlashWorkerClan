...
import kotlinx.android.synthetic.main.activity_settlement_salary.*
 ...
 * -----------------------------------------------------------------
 * File: SettlementSalaryActivity
 * Author: Victor
 * Date: 2020/12/17 20:41
 * Description: 
 * -----------------------------------------------------------------
 */
class SettlementSalaryActivity: BaseActivity(),View.OnClickListener, OnTradePasswordListener,
        AdapterView.OnItemClickListener {

    var mLoadingDialog: LoadingDialog? = null
    var passwordErrorCount: Int = 0//密码输入错误次数

    var mSettlementConfirmDetailParm: SettlementConfirmDetailParm? = null
    var mSettlementConfirmDetailData: SettlementConfirmDetailData? = null

    var mSalaryTalentAdapter: SalaryTalentAdapter? = null

    companion object {
        fun  intentStart (activity: AppCompatActivity,data: SettlementConfirmDetailParm?) {
            var intent = Intent(activity, SettlementSalaryActivity::class.java)
    ...
    override fun getLayoutResource() = R.layout.activity_settlement_salary
    ...
    fun initData (intent: Intent?) {
        mSettlementConfirmDetailParm = intent?.getSerializableExtra(Constant.INTENT_DATA_KEY)
                as SettlementConfirmDetailParm
    ...
    fun subscribeUi() {
        ...
        employmentVM.settlementConfirmDetailData.observe(this, Observer {
            mLoadingDialog?.dismiss()
            when(it) {
                is HttpResult.Success -> {
                    showSettlementConfirmDetail(it.value.data)
                }
                is HttpResult.Error -> {
                    ToastUtils.show(it.message.toString())
                }
        ...
        employmentVM.settlementData.observe(this, Observer {
            mLoadingDialog?.dismiss()
            when(it) {
                is HttpResult.Success -> {
                    SettlementSalarySuccessActivity.intentStart(this,mSettlementConfirmDetailData)
    ...
    fun sendSettlementConfirmDetailRequest () {
        ...
        employmentVM.fetchSettlementConfirmDetail(token,mSettlementConfirmDetailParm)
    }

    fun sendSettlementRequest (tradePassword: String?) {
        mLoadingDialog?.show()
        val loginReq = App.get().getLoginReq()
        val token = loginReq?.data?.token

        val body = SettlementParm()
        body.settlementOrderIds = mSettlementConfirmDetailParm?.settlementOrderIds
        body.employerReleaseId = mSettlementConfirmDetailParm?.employerReleaseId
        body.tradePassword = tradePassword

        employmentVM.settlement(token,body)
    }

    fun showSettlementConfirmDetail (data: SettlementConfirmDetailData?) {
        mSettlementConfirmDetailData = data
        ...
        mSalaryTalentAdapter?.add(data?.settlementUsers)
    ...
    fun showSetTransactionPwdDlg () {
        ...
        commonTipDialog.mOnDialogOkCancelClickListener = object : OnDialogOkCancelClickListener {
            override fun OnDialogOkClick() {
                NavigationUtils.goSetTransactionPasswordActivity(this@SettlementSalaryActivity)
            }
    ...
    fun showErrorPasswordTipDlg () {
        ...
        commonTipDialog.mOnDialogOkCancelClickListener = object : OnDialogOkCancelClickListener {
            ...
            override fun OnDialogCancelClick() {
                ...
                NavigationUtils.goVerifyIdentidyActivity(this@SettlementSalaryActivity)
            }
    ...
    fun showAuthTipDlg () {
        ...
        commonTipDialog.mOnDialogOkCancelClickListener = object : OnDialogOkCancelClickListener {
            override fun OnDialogOkClick() {
                NavigationUtils.goRealNameActivity(this@SettlementSalaryActivity)
            }
    ...
    override fun OnTradePassword(tradeAmount: Double, tradePassword: String?) {
        sendSettlementRequest(tradePassword)
    }
