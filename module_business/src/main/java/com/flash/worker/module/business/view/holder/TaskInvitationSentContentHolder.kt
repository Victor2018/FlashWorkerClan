...
import kotlinx.android.synthetic.main.rv_task_invitation_sent_cell.view.*
 ...
 * -----------------------------------------------------------------
 * File: TaskInvitationSentContentHolder
 * Author: Victor
 * Date: 2020/12/19 11:38
 * Description: 
 * -----------------------------------------------------------------
 */
class TaskInvitationSentContentHolder(itemView: View) : ContentViewHolder(itemView) {
    ...
    fun bindData (data: EmployerJobInviteInfo?) {
        ...
        itemView.mTvTitle.text = "${data?.title}(${data?.settlementTimeLimit}小时内结算)"
    }
