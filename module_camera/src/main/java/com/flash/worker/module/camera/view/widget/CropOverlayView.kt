...
import com.flash.worker.module.camera.cropper.CropPosition
...
class CropOverlayView: View {
    ...
    private var topRight:android.graphics.Point? = null
    private var bottomLeft:android.graphics.Point? = null
    private var bottomRight:android.graphics.Point? = null

    private var touchDownX = 0f
    private var touchDownY:kotlin.Float = 0f
...
//        drawGrid(canvas);//裁剪框内部线条
    ...
    private fun drawGrid(canvas: Canvas) {
        ...
        for (i in 1..gridSize) {
            ...
            val topDistanceY =
                Math.abs((topLeft!!.y - topRight!!.y) / (gridSize + 1) * i)
            val top = Point(
            ...
            val bottomDistanceX =
                Math.abs((bottomLeft!!.x - bottomRight!!.x) / (gridSize + 1) * i)
            val bottomDistanceY =
                Math.abs((bottomLeft!!.y - bottomRight!!.y) / (gridSize + 1) * i)
            val bottom = Point(
            ...
            val leftDistanceX =
                Math.abs((topLeft!!.x - bottomLeft!!.x) / (gridSize + 1) * i)
            val leftDistanceY =
                Math.abs((topLeft!!.y - bottomLeft!!.y) / (gridSize + 1) * i)
            val left = Point(
            ...
            val rightDistanceX =
                Math.abs((topRight!!.x - bottomRight!!.x) / (gridSize + 1) * i)
            val rightDistanceY =
                Math.abs((topRight!!.y - bottomRight!!.y) / (gridSize + 1) * i)
            val right = Point(
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        when (event!!.action) {
            MotionEvent.ACTION_UP -> parent.requestDisallowInterceptTouchEvent(
                false
            )
    ...
    fun crop(cropListener: CropListener, needStretch: Boolean) {
        ...
        if (!needStretch) {
            ...
            val stretch = Bitmap.createBitmap(
                cut.width,
                cut.height,
                Bitmap.Config.ARGB_8888
            )
    ...
    private fun generateVertices(widthBitmap: Int, heightBitmap: Int): FloatArray? {
        val vertices =
            FloatArray((WIDTH_BLOCK + 1) * (HEIGHT_BLOCK + 1) * 2)
