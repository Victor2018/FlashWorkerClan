...
import kotlinx.android.synthetic.main.activity_salary_payment_detail.*

class SalaryPaymentDetailActivity : BaseActivity(), View.OnClickListener {

    companion object {
        fun  intentStart (activity: AppCompatActivity,data: BalanceFlowInfo?) {
            var intent = Intent(activity, SalaryPaymentDetailActivity::class.java)
    ...
    override fun getLayoutResource() = R.layout.activity_salary_payment_detail
    ...
    fun showBalanceFlowDetail (data: BalanceFlowDetailReq) {
        ...
        mTvPayoutAmount.text = "${AmountUtil.addCommaDots(data.data?.actualAmount)}元"
