...
import kotlinx.android.synthetic.main.activity_credit_compensation_detail.*

class CreditCompensationDetailActivity : BaseActivity(), View.OnClickListener {

    companion object {
        fun  intentStart (activity: AppCompatActivity,data: BalanceFlowInfo?) {
            var intent = Intent(activity, CreditCompensationDetailActivity::class.java)
    ...
    override fun getLayoutResource() = R.layout.activity_credit_compensation_detail
    ...
    fun showBalanceFlowDetail (data: BalanceFlowDetailReq) {
        ...
        mTvCompensationAmount.text = "${AmountUtil.addCommaDots(data.data?.actualAmount)}元"
