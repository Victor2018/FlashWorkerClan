...
class WithdrawDetailActivity : BaseActivity(), View.OnClickListener {

    companion object {
        fun  intentStart (activity: AppCompatActivity,data: BalanceFlowInfo?) {
            var intent = Intent(activity, WithdrawDetailActivity::class.java)
    ...
    override fun getLayoutResource() = R.layout.activity_withdraw_detail
    ...
    fun showBalanceFlowDetail (data: BalanceFlowDetailReq) {
        ...
        mTvServiceAmount.text = "${AmountUtil.addCommaDots(data.data?.feeAmount)}元"
        mTvApplyTime.text = data.data?.tradeTime
        mTvArrivalTime.text = data.data?.finishTime
        mTvTradeNo.text = data.data?.outTradeNo

        if (TextUtils.equals(data.data?.channel,PayChannel.WX_PAY)) {
            mTvWithdrawChannel.text = "微信"
        } else if (TextUtils.equals(data.data?.channel,PayChannel.ALI_PAY)) {
            mTvWithdrawChannel.text = "支付宝"
        }
    }
