...
import kotlinx.android.synthetic.main.activity_service_fee_detail.*

class ServiceFeeDetailActivity : BaseActivity(), View.OnClickListener {

    companion object {
        fun  intentStart (activity: AppCompatActivity,data: BalanceFlowInfo?) {
            var intent = Intent(activity, ServiceFeeDetailActivity::class.java)
    ...
    override fun getLayoutResource() = R.layout.activity_service_fee_detail
    ...
    fun showBalanceFlowDetail (data: BalanceFlowDetailReq) {
        ...
        mTvServiceFeeAmount.text = "${AmountUtil.addCommaDots(data.data?.actualAmount)}元"
