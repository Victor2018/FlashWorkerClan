...
import kotlinx.android.synthetic.main.activity_salary_expense_detail.*

class SalaryExpenseDetailActivity : BaseActivity(), View.OnClickListener {

    companion object {
        fun  intentStart (activity: AppCompatActivity,data: BalanceFlowInfo?) {
            var intent = Intent(activity, SalaryExpenseDetailActivity::class.java)
    ...
    override fun getLayoutResource() = R.layout.activity_salary_expense_detail
    ...
    fun showBalanceFlowDetail (data: BalanceFlowDetailReq) {
        ...
        mTvSettlementAmount.text = "${AmountUtil.addCommaDots(data.data?.actualAmount)}元"
