...
import kotlinx.android.synthetic.main.act_member_detail_content_header.*
import kotlinx.android.synthetic.main.act_member_detail_header.*
import kotlinx.android.synthetic.main.activity_member_detail.*

class MemberDetailActivity : BaseActivity(), View.OnClickListener,
        SwipeRefreshLayout.OnRefreshListener,AppBarLayout.OnOffsetChangedListener {

    companion object {
        fun  intentStart (activity: AppCompatActivity, data: GuildMemberInfo?) {
    ...
    var mGuildMemberInfo: GuildMemberInfo? = null

    override fun getLayoutResource() = R.layout.activity_member_detail
    ...
    fun initData (intent: Intent?) {
        mGuildMemberInfo = intent?.getSerializableExtra(Constant.INTENT_DATA_KEY) as GuildMemberInfo?
        sendMemberDetailRequest()
    }

    fun subscribeUi () {
        guildVM.memberDetailData.observe(this, Observer {
            mSrlRefresh.isRefreshing = false
            when(it) {
                is HttpResult.Success -> {
                    showMemberDetailData(it.value)
                }
                is HttpResult.Error -> {
    ...
    fun sendMemberDetailRequest () {
        ...
        val body = MemberDetailParm()
        body.guildId = mGuildMemberInfo?.guildId
        body.userId = mGuildMemberInfo?.userId

        guildVM.fetchMemberDetail(token,body)
    }
    ...
    fun sendImLoginInfoRequest () {
        ...
        userVM.fetchImLoginInfo(token,mGuildMemberInfo?.userId)
    }

    fun showMemberDetailData(data: MemberDetailReq) {
        ...
        if (height > 0) {
            mTvHeight.text = "${data?.data?.height}cm"
        ...
        if (weight > 0) {
            mTvWeight.text = "${data?.data?.weight}kg"
        ...
        mTvJoinTime.text = "入会时间：${data?.data?.joinTime}"
    }
    ...
    override fun onClick(v: View?) {
        when (v?.id) {
            ...
            R.id.mTvUserId -> {
                val userId = mGuildMemberInfo?.userId
