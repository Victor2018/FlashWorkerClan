...
import kotlinx.android.synthetic.main.rv_message_image_send_cell.view.*

/*
 * -----------------------------------------------------------------
 * Copyright (C) 2020-2080, by Victor, All rights reserved.
 * -----------------------------------------------------------------
 * File: MessageImageSendContentHolder
 * Author: Victor
 * Date: 2020/12/30 10:34
 * Description: 
 * -----------------------------------------------------------------
 */
class MessageImageSendContentHolder(itemView: View) : ContentViewHolder(itemView) {
    ...
    fun bindData (data: IMMessage?) {
        ...
        if (msgAttachment is ImageAttachment) {
            var path = msgAttachment.path
            if (TextUtils.isEmpty(path)) {
                path = msgAttachment.thumbUrl
            }
            ImageUtils.instance.loadImage(itemView.context,itemView.mIvMessage,path)
        }
